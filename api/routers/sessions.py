...
async def get_session(session_id: str, iam: AgenticIAM = Depends(get_iam)):
    """
...
async def refresh_session(
    session_id: str, iam: AgenticIAM = Depends(get_iam), settings: Settings = Depends(get_settings)
):
    """
    Refresh a session
    ...
    try:
        ...
        if iam.audit_manager:
            ...
            await iam.audit_manager.log_event(
                ...
                agent_id=session.agent_id,
                details={"session_id": session_id, "updated_metadata": request.metadata},
            )
...
async def terminate_sessions(request: SessionTerminateRequest, iam: AgenticIAM = Depends(get_iam)):
    """
    ...
    try:
        ...
        return SuccessResponse(
            ...
            data={"terminated_count": terminated_count, "reason": request.reason},
        )
...
async def get_session_stats(iam: AgenticIAM = Depends(get_iam)):
    """
...
@router.post("/cleanup")
async def cleanup_expired_sessions(iam: AgenticIAM = Depends(get_iam)):
    """
    ...
    try:
        ...
        if iam.audit_manager:
            ...
            await iam.audit_manager.log_event(
                ...
                details={"operation": "session_cleanup", "cleaned_sessions": cleaned_count},
            )
