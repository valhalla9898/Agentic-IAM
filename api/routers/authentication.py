    ...
    try:
        ...
        if method_impl and agent_entry:
            try:
                ...
                logging.getLogger(__name__).debug("Token generation failed: %s", e)
                token = None
...
async def logout(
    ...
    try:
        ...
        return {
            ...
            "data": {"terminated_sessions": terminated},
        }
...
async def get_methods(iam: AgenticIAM = Depends(get_iam), settings=Depends(get_settings)):
    ...
    return {
        ...
        "mfa_enabled": getattr(settings, "enable_mfa", False),
        "federated_auth_enabled": getattr(settings, "enable_federated_auth", False),
    }
...
async def mfa_start(
    ...
    try:
        ...
        result = (
            mfa_impl.start_authentication(agent_id)
            if not callable(getattr(mfa_impl.start_authentication, "__await__", None))
            else await mfa_impl.start_authentication(agent_id)
        )
        ...
        return {
            ...
            "required_factors": (
                ...
                if isinstance(result, dict)
                else getattr(result, "required_factors", None)
            ),
            "available_methods": (
                result.get("available_methods")
                if isinstance(result, dict)
                else getattr(result, "available_methods", None)
            ),
            "expires_in": 600,
        }
    except HTTPException:
