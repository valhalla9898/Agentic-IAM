...
async def login(payload: LoginRequest, iam: AgenticIAM = Depends(get_iam)):
    try:
        ...
        token = None
        method_impl = getattr(
            iam.authentication_manager, "methods", {}).get(
    ...
    try:
        if session_id:
            ...
            raise HTTPException(status_code=400,
                                detail="Either session_id or agent_id must be provided")

        return {"message": f"Successfully terminated {terminated} session(s)", "data": {
    ...
    try:
        ...
        if not crypto:
            raise HTTPException(status_code=501,
                                detail="Cryptographic authentication not available")
...
async def get_methods(iam: AgenticIAM = Depends(get_iam), settings=Depends(get_settings)):
    ...
    return {
        ...
        "mfa_enabled": getattr(
            ...
            "enable_mfa",
            False),
        "federated_auth_enabled": getattr(
            settings,
            "enable_federated_auth",
            False)}
...
async def mfa_start(
    ...
    try:
        ...
        return {
            "mfa_session_id": result.get("session_id") if isinstance(
                result,
                dict) else getattr(
                result,
                "session_id",
                None),
            "required_factors": result.get("required_factors") if isinstance(
                result,
                dict) else getattr(
                    result,
                    "required_factors",
                    None),
            "available_methods": result.get("available_methods") if isinstance(
                result,
                dict) else getattr(
                result,
                "available_methods",
...
async def get_status(agent_id: str, iam: AgenticIAM = Depends(get_iam)):
    try:
        ...
        audit_events = iam.audit_manager.query_events(
            agent_id=agent_id) if getattr(
            iam, "audit_manager", None) else []
