            ...
            with self.get_connection() as conn:
                ...
                if count == 0:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, ("admin", admin_password, "admin", "System Administrator", "admin@agentic-iam.local"))
                    ...
                    cursor.execute("""
                        INSERT INTO users (username, password, role, full_name, email)
                        VALUES (?, ?, ?, ?, ?)
                    """, ("user", user_password, "user", "Standard User", "user@agentic-iam.local"))
                    
                    conn.commit()
                    logger.info("Default users created successfully (admin/admin123, user/user123)")
        except Exception as e:
    ...
    def list_users(self) -> List[Dict]:
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    SELECT id, username, role, full_name, email, created_at, last_login, status 
                    FROM users 
                    ORDER BY created_at DESC
                """)
                rows = cursor.fetchall()
