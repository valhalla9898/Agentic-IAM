...
class AgenticIAM:
        ...
        try:
            ...
            if not self.agent_registry:
                self.agent_registry = AgentRegistry(
                    storage_path=self.settings.agent_registry_path, enable_persistence=True
                )
            ...
            await self.authorization_manager.initialize(
                enable_rbac=True, enable_abac=True, enable_pbac=True, hybrid_mode=True
            )
            ...
            await self.transport_manager.initialize(
                enable_http=True, enable_grpc=True, enable_websocket=True, enable_stdio=True
            )
            ...
            await self.compliance_manager.initialize(frameworks=["gdpr", "hipaa", "sox", "pci_dss"])
    ...
    async def register_agent(
        self, agent_identity: AgentIdentity, initial_permissions: Optional[List[str]] = None
    ) -> str:
    ...
    def delete_agent(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # DB sync is best-effort for non-dashboard runtimes; log failure
            self.logger.debug(f"DB sync during agent deletion failed: {e}")
            db_deleted = True
    ...
    async def authenticate(
        self, agent_id: str, credentials: Dict[str, Any], method: str = "auto", **kwargs
        ...
        try:
            ...
            result = await self.authentication_manager.authenticate(
                agent_id=agent_id, credentials=credentials, method=method, **kwargs
            )
            ...
            if self.intelligence_engine and result.success:
                await self.intelligence_engine.update_trust_score(
                    agent_id=agent_id, event_type="authentication_success", context=kwargs
                )
    ...
    async def authorize(
        self, agent_id: str, resource: str, action: str, context: Optional[Dict[str, Any]] = None
    ) -> bool:
        ...
        try:
            ...
            decision = await self.authorization_manager.authorize(
                agent_id=agent_id, resource=resource, action=action, context=context or {}
            )
