...
def _fetch_jwks(issuer: str):
    ...
    try:
        ...
        logging.getLogger(__name__).debug("Failed to fetch JWKS for issuer %s: %s", issuer, e)
        return None
...
def verify_oidc_token(token: str, issuer: str = None, audience: str = None) -> dict | None:
    ...
    try:
        ...
        logging.getLogger(__name__).debug("Unexpected error verifying OIDC token: %s", e)
        return None
